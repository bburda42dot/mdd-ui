...
use super::services::get_parent_ref_services_recursive;
use crate::tree::{
    builder::TreeBuilder,
    types::{
...
/// Add DOPs section to the tree by collecting from service request/response params
pub fn add_dops_section<'a>(
    b: &mut TreeBuilder,
    layer: &DiagLayer<'a>,
    depth: usize,
    parent_refs: Option<impl Iterator<Item = ParentRef<'a>> + Clone>,
) {
    let mut all_dops: Vec<DopInfo<'a>> = Vec::new();

    // Collect DOPs from own services
    if let Some(services) = layer.diag_services() {
        for svc in services.iter() {
            let ds = DiagService(svc);
            collect_dops_from_service(&ds, &mut all_dops);
        }
    }

    // Collect DOPs from inherited services (via parent refs)
    if let Some(refs) = parent_refs {
        let parent_services = get_parent_ref_services_recursive(refs);
        for (ds, _source) in &parent_services {
            collect_dops_from_service(ds, &mut all_dops);
        }
    }

    // Deduplicate DOPs by short name
    let mut seen_names = std::collections::HashSet::new();
    let unique_dops: Vec<_> = all_dops
        .into_iter()
        .filter(|dop_info| {
            if seen_names.contains(&dop_info.name) {
                false
            } else {
                seen_names.insert(dop_info.name.clone());
                true
            }
        })
        .collect();
    ...
    // Group DOPs by semantic category: DTC-DOPS, ENV-DATA-DESCS, DATA-OBJECT-PROPS
    ...
    for dop_info in unique_dops {
        match dop_info.dop_type.as_str() {
            "DTCDOP" => dtc_dops.push(dop_info),
            "EnvDataDesc" | "EnvData" => env_data_descs.push(dop_info),
            _ => data_object_props.push(dop_info),
        }
    }
    ...
    // Add DIAG-DATA-DICTIONARY-SPEC section header (yellow)
    b.push_details_structured(
        depth,
        "DIAG-DATA-DICTIONARY-SPEC".to_owned(),
        false,
        true,
        dops_detail,
        NodeType::DOP,
...
fn collect_dop_from_param<'a>(
    ...
    if let Some(dop) = raw_dop {
        if let Some(name) = dop.short_name() {
            let specific_type_raw = format!("{:?}", dop.specific_data_type());
            let dop_type = specific_type_raw.trim_matches('"').to_owned();

            let dop_wrap = cda_database::datatypes::DataOperation(dop);

            // Extract category, units, and desc_id based on variant type
            let (category, internal_unit, phys_unit, desc_id) = if let Ok(variant) = dop_wrap.variant() {
                ...
                match variant {
                    ...
                    DataOperationVariant::EnvDataDesc(env_desc) => {
                        let did = env_desc.param_short_name()
                            .map(|s| s.to_owned());
                        (None, None, None, did)
                    }
...
fn collect_dops_from_service<'a>(ds: &DiagService<'a>, all_dops: &mut Vec<DopInfo<'a>>) {
    // Collect from request params
    if let Some(request) = ds.request() {
        if let Some(params) = request.params() {
            for param in params.iter() {
                let pw = Parameter(param);
                collect_dop_from_param(&pw, all_dops);
            }
        }
    }
...
fn build_dop_detail_sections(dop_info: &DopInfo<'_>) -> Vec<DetailSectionData> {
    ...
    // Add parsed information from name
    ...
    if let Ok(variant) = dop_info.dop.variant() {
        match variant {
            DataOperationVariant::Normal(normal_dop) => {
                build_normal_dop_tabs(&normal_dop, &parsed_name, &mut sections);
            }
            DataOperationVariant::Structure(structure) => {
                build_structure_dop_tabs(&structure, &mut sections);
            }
            DataOperationVariant::StaticField(static_field) => {
                build_static_field_dop_tabs(&static_field, &mut sections);
            }
            DataOperationVariant::EndOfPdu(eof_field) => {
                build_end_of_pdu_dop_tabs(&eof_field, &mut sections);
            }
            DataOperationVariant::DynamicLengthField(dyn_field) => {
                build_dynamic_length_field_dop_tabs(&dyn_field, &mut sections);
            }
            DataOperationVariant::EnvDataDesc(env_desc) => {
                build_env_data_desc_dop_tabs(&env_desc, &mut sections);
            }
            DataOperationVariant::EnvData(env_data) => {
                build_env_data_dop_tabs(&env_data, &mut sections);
            }
            DataOperationVariant::Mux(mux_dop) => {
                build_mux_dop_tabs(&mux_dop, &mut sections);
            }
            DataOperationVariant::Dtc(dtc_dop) => {
                build_dtc_dop_tabs(&dtc_dop, &mut sections);
            }
        }
    }
...
fn build_normal_dop_tabs(
    ...
    // Tab 1: Types (DiagCodedType, PhysicalType)
    ...
    if !constraints_rows.is_empty() {
        ...
        sections.push(DetailSectionData {
            title: "Internal Constraints".to_owned(),
            render_as_header: false,
            section_type: DetailSectionType::Custom,
            content: DetailContent::Table {
...
fn build_structure_dop_tabs(
    ...
    if !structure_rows.is_empty() {
        ...
        sections.push(DetailSectionData {
            title: "Structure Details".to_owned(),
            render_as_header: false,
            section_type: DetailSectionType::Custom,
            content: DetailContent::Table {
                header,
...
fn build_static_field_dop_tabs(
    ...
    if !field_rows.is_empty() {
        ...
        sections.push(DetailSectionData {
            title: "Static Field Details".to_owned(),
            render_as_header: false,
            section_type: DetailSectionType::Custom,
            content: DetailContent::Table {
                header,
...
fn build_end_of_pdu_dop_tabs(
    ...
    if !eof_rows.is_empty() {
        ...
        sections.push(DetailSectionData {
            title: "End of PDU Details".to_owned(),
            render_as_header: false,
            section_type: DetailSectionType::Custom,
            content: DetailContent::Table {
                header,
...
fn build_dynamic_length_field_dop_tabs(
    ...
    if !dyn_rows.is_empty() {
        ...
        sections.push(DetailSectionData {
            title: "Dynamic Length Field Details".to_owned(),
            render_as_header: false,
            section_type: DetailSectionType::Custom,
            content: DetailContent::Table {
                header,
...
fn build_env_data_desc_dop_tabs(
    ...
    if !env_rows.is_empty() {
        ...
        sections.push(DetailSectionData {
            title: "Env Data Desc Details".to_owned(),
            render_as_header: false,
            section_type: DetailSectionType::Custom,
            content: DetailContent::Table {
                header,
...
fn build_env_data_dop_tabs(
    ...
    if !env_rows.is_empty() {
        ...
        sections.push(DetailSectionData {
            title: "Env Data Details".to_owned(),
            render_as_header: false,
            section_type: DetailSectionType::Custom,
            content: DetailContent::Table {
                header,
...
fn build_dtc_dop_tabs(
    ...
    let mut dtc_rows = Vec::new();
    
    if let Ok(coded_type) = dtc_dop.diag_coded_type() {
        dtc_rows.push(DetailRow {
            cells: vec!["Diag Coded Type".to_owned(), format!("{:?}", coded_type.base_datatype())],
            cell_types: vec![CellType::Text, CellType::Text],
            indent: 0,
            row_type: DetailRowType::Normal,
