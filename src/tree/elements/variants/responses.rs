...
pub fn build_pos_responses_sections(ds: &DiagService<'_>) -> Vec<DetailSectionData> {
    ...
    if let Some(pos) = ds.pos_responses() {
        for (i, resp) in pos.iter().enumerate() {
            ...
            sections.push(build_response_param_section(&format!("Pos-Response {}", i + 1), params));
        }
    }
...
pub fn build_neg_responses_sections(ds: &DiagService<'_>) -> Vec<DetailSectionData> {
    ...
    if let Some(neg) = ds.neg_responses() {
        for (i, resp) in neg.iter().enumerate() {
            let params = resp.params().into_iter().flatten().map(Parameter);
            sections.push(build_response_param_section(&format!("Neg-Response {}", i + 1), params));
        }
    }
...
fn build_pos_response_view_sections(ds: &DiagService<'_>, parent_layer_name: Option<String>) -> Vec<DetailSectionData> {
    let mut sections = Vec::new();
...
fn build_neg_response_view_sections(ds: &DiagService<'_>, parent_layer_name: Option<String>) -> Vec<DetailSectionData> {
    let mut sections = Vec::new();
...
fn build_overview_section(ds: &DiagService<'_>, parent_layer_name: Option<String>) -> DetailSectionData {
